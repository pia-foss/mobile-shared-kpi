...
class KPIAPITest {
    ...
    @Test
    fun `Test undefined preferences name`() {
        assertFailsWith(Exception::class) {
            KPIBuilder()
    ...
    @Test
    fun `Test invalid events batch size`() {
        assertFailsWith(Exception::class) {
            KPIBuilder()
                .setPreferenceName("test-preference-name")
                .setCertificate("test-certificate")
                .setKPIFlushEventMode(KPISendEventsMode.PER_BATCH)
                .setKPIClientStateProvider(MockClientStateProvider())
                .setEventsBatchSize(0)
                .build()
        }
    }

    @Test
    fun `Test invalid events history size`() {
        assertFailsWith(Exception::class) {
            KPIBuilder()
                .setPreferenceName("test-preference-name")
                .setCertificate("test-certificate")
    ...
    @Test
    fun `Test invalid requests timeout`() {
        assertFailsWith(Exception::class) {
            KPIBuilder()
                .setPreferenceName("test-preference-name")
                .setCertificate("test-certificate")

...
import com.privateinternetaccess.kpi.internals.utils.KPIUtils.snakeToPascalCase
...
internal expect class KPIPlatformProvider(
    ...
    internal val preferences: KPIPreferences

    internal val logger: KPILogger

    internal val httpLogLevel: KPIHttpLogLevel
}
...
internal class KPI(
    ...
    private val requestFormat: KPIRequestFormat,
    preferenceName: String,
    userAgent: String?,
    ...
    private val kpiProvider = KPIPlatformProvider(preferenceName, userAgent, isLoggingEnabled, httpLogLevel)
    private val kpiPersistency: IKPIPersistency = KPIPersistencyImpl(kpiProvider)
    private val kpiEventUtils: KPIEventUtils = KPIEventUtils(kpiPersistency, kpiClientStateProvider.projectToken())
    ...
    private fun stopAsync(callback: (error: KPIError?) -> Unit) {
        ...
        try {
            ...
            kpiProvider.logger.logError(tag = TAG, message = stacktrace)
    ...
    private suspend fun submitAsync(
        ...
        if (error == null) {
            try {
                ...
                val stacktrace = t.stackTraceToString()
                kpiProvider.logger.logDebug(tag = TAG, message = stacktrace)
                error = KPIError(description = stacktrace)
            }
        }
    ...
    private suspend fun sendEvents(
        ...
        if (!started) {
            error = KPIError("KPI has not been started. Events will not be sent.")
        }
        ...
        if (events.isEmpty()) {
            error = KPIError("There are no events in queue. Skipping request.")
        }
            ...
            for (endpoint in endpoints) {
                ...
                val client = if (endpoint.usePinnedCertificate) {
                    KPIHttpClient.client(
                        kpiHttpLogLevel = kpiProvider.httpLogLevel,
                ...
                val response: Pair<HttpResponse?, Throwable?> = when (requestFormat) {
                    KPIRequestFormat.KAPE -> client.postCatching {
                        ...
                        setBody(
                            KPIPlatformUtils.encodeWithKapeFormat(
                                events = events,
                    ...
                    KPIRequestFormat.ELASTIC -> client.postCatching {
                        ...
                        setBody(
                            KPIPlatformUtils.encodeWithElasticFormat(
                                events = events,
    ...
    private suspend fun recentEventsAsync(callback: (events: List<String>) -> Unit) {
        ...
        for (event in sampleEvents) {
            ...
            result += "EventToken: ${kpiClientStateProvider.projectToken()} "
            event.eventProperties.map { (key, value) -> result += "EventProperties.${key.snakeToPascalCase()}: $value ".trim() }
            result += ""
        }

...
internal class KPIPersistencyImpl internal constructor(
    ...
    override fun persistIdentifier(identifier: KPIEventIdentifier) {
        try {
            val kpiPreferences: KPIPreferences? = kpiProvider.kpiPreferences
            if (kpiPreferences?.isValid != true) {
                kpiProvider.kpiLogger.logDebug(tag = TAG, message = "KPIPreferences are invalid")
                return
            }
            ...
            kpiProvider.kpiLogger.logError(tag = TAG, message = t.stackTraceToString())
        }
    }
    ...
    override fun identifier(): KPIEventIdentifier? {
        try {
            ...
            return kpiPreferences.getString(key = AGGREGATED_ID_PERSISTENCY_KEY, default = null)?.let { value ->
    ...
    private fun persistedEventsForKey(key: String): List<KPIEvent> {
        ...
        if (encodedEvents.isBlank()) {
            kpiProvider.kpiLogger.logDebug(tag = TAG, message = "KPIPreferences: no data for key '$key'")
            return emptyList()
        }
        return json.decodeFromString(ListSerializer(KPIEvent.serializer()), encodedEvents)
    }
    // endregion
}